...
def plot_batch(X, out_path):
    ...
    for i in range(np.shape(X)[0]):
        PIL.Image.fromarray((X[i] * 255).astype(np.uint8)).save(out_path)
...
class IndexFlow(object):
    ...
    def __init__(
            ...
            fill_batches=True,
            shuffle=True,
            return_keys=["imgs", "joints", "norm_imgs", "norm_joints", "out_name"]):
        self.shape = shape
        ...
        self.indices = np.array(
            [i for i in range(len(self.index["test"]))
             if self._filter(i)])
    ...
    def _filter(self, i):
        good = True
        good = good and (self.index["test"][i] == self.train)
    ...
    def __next__(self):
        ...
        for i in batch_indices:
            batch["out_name"] = self.index["out_name"][i]
