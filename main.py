...
#***********# added on 12/2/2021 to bypass error about eager execution
...
class Model(object):
    ...
    def define_models(self):
        ...
        n_filters = 32
        self.enc_up_pass = models_new.make_model(
                "enc_up", models_new.enc_up,
                n_scales = n_scales - self.box_factor,
                n_filters = n_filters*2**self.box_factor)
        self.enc_down_pass = models_new.make_model(
                "enc_down", models_new.enc_down,
                n_scales = n_scales - self.box_factor,
                n_latent_scales = n_latent_scales)
        self.dec_up_pass = models_new.make_model(
                "dec_up", models_new.dec_up,
                n_scales = n_scales,
                n_filters = n_filters)
        self.dec_down_pass = models_new.make_model(
                "dec_down", models_new.dec_down,
                n_scales = n_scales,
                n_latent_scales = n_latent_scales)
        self.dec_params = models_new.make_model(
                "dec_params", models_new.dec_parameters)
        ##add
        self.discrim = models_new.make_model(
                "discrim", models_new.discriminator)
    ...
    def define_graph(self):
        ...
        # add: get discriminator results and get loss
        r_logits, r_rep = self.discrim(self.x)
        f_logits, g_rep = self.discrim(params,reuse=True)
        ...
        for q, p in zip(qs, ps):
            ...
            kl_loss += models_new.latent_kl(q, p)
        #add
        loss = 0.5* likelihood_loss + kl_weight * kl_loss + 0.5*(disc_loss + gen_loss)
        #org: loss = 0.5* likelihood_loss + kl_weight * kl_loss + 0.5*(disc_loss + gen_loss)
...
if __name__ == "__main__":
    ...
    if opt.mode == "train":
        ...
        data_shape = [batch_size] + img_shape
        box_factor = config["box_factor"]
        valid_batches = get_batches(data_shape, config['data_index'],box_factor = box_factor, train = False)
        model = Model(config, out_dir, logger)
        model.restore_graph(opt.checkpoint)
        ...
        for i in trange(valid_batches.n // batch_size):
            X_batch, C_batch, XN_batch, CN_batch, out_name = next(valid_batches)
